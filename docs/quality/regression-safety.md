...
### Docker 가 켜져 있는데도 skip 되던 문제 (#92)

로컬(Windows + Docker Desktop, Engine 29)에서는 Docker 가 켜져 있어도 skip 됐습니다.

| 요청 API 버전 | 결과 |
|---------------|------|
| 1.32 | **거부** (빈 `/info` 를 400 으로 반환) |
| 1.40 / 1.44 | 정상 |

Testcontainers 가 쓰는 docker-java 는 API 버전을 따로 주지 않으면 **1.32 로 고정**합니다.
Engine 29 는 최소 API 가 1.40 이라 이 요청을 거부하고, Testcontainers 는 이를 "Docker 를 찾지 못함" 으로
판단해 테스트를 건너뜁니다. Testcontainers 를 1.21.4, 2.0.5 로 올려도 같았습니다.

`build.gradle` 의 테스트 태스크에서 `api.version=1.44` 를 넘겨 해결했습니다. 의존성은 바꾸지 않았습니다.
1.44 는 Engine 25 이상이 지원하므로 CI 에도 안전합니다. 다르게 주려면 `./gradlew test -Papi.version=1.45`.

**CI 도 같은 시한폭탄을 안고 있었습니다.** 지금 CI 가 통과하는 건 러너가 아직 이전 Engine 이기 때문이고,
러너가 29 로 올라가면 위 검사가 빌드를 실패시켰을 겁니다.

### 로컬에서도 알린다

로컬은 CI 처럼 실패시키지 않습니다 — Docker 없이도 나머지 테스트는 돌려야 합니다.
대신 skip 되면 테스트 끝에 경고를 띄웁니다.

```
[경고] 스키마 정합성 검증(FlywaySchemaValidationTest)이 skip 됐습니다.
    Docker 가 없거나 연결되지 않았습니다. 마이그레이션과 엔티티가 어긋나도 이 빌드는 잡지 못합니다.
```
